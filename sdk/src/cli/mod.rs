...
>(
    ...
    match cli.command {
        ...
        SnarkCmd::Keygen => {
            let srs = read_srs_from_dir(&cli.srs, runner.k() as u32).expect("Unable to read SRS");
            ...
            let metadata = if should_aggregate {
                ...
                let agg_kzg_params =
                    read_srs_from_dir(&cli.srs, agg_circuit_params.unwrap().degree)
        ...
        SnarkCmd::Run => {
            ...
            let srs = read_srs_from_dir(&cli.srs, prover.k() as u32).expect("Unable to read SRS");
            ...
            let output = if should_aggregate {
                ...
                let (agg_pk, agg_pinning) =
                    read_agg_pk_and_pinning(cli.data_path.clone(), agg_circuit_id);
                let agg_srs = read_srs_from_dir(&cli.srs, agg_pinning.params.degree)
                    .expect("Unable to read SRS");
